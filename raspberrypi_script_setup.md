...
Think of this server as an isolated factory. We are going to install the required network tools, create a safe sandbox for Python, and set up the automatic alarm clocks (Cron) so your scripts run forever without overloading your main web database server.

## Step 1: Install the System Tools
Before Python can scan your network, the operating system needs the raw networking tools to ping, trace, and scan.

Log into your server via SSH and run this command to update the system and install the required tools:
...
## CODE:
mkdir -p /home/jamie/lanman_worker
cd /home/jamie/lanman_worker
python3 -m venv venv
